...
function detectCurrentRepo() {
  ...
  const repo = String(output ?? "").trim();
  if (!repo) {
    throw new Error("Could not detect current repository. Run from a git repo with gh auth, or pass --repo <owner/name>.");
  }
  return repo;
}
...
function printHelp() {
  ...
  console.log("");
  console.log("If --repo is omitted, the script targets the current gh repository.");
}
...
function main() {
  ...
  if (args.command === "list") {
    ...
    printThreads({ pr: args.pr, includeResolved: args.includeResolved, repo });
    return;
  }
