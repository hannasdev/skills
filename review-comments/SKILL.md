...
6. Keep changes scoped to the reviewed concern.
...
- Do not create unsigned commits by default; review-response commits should include a `Signed-off-by:` trailer.
...
- For commit commands during review response, prefer `git commit -s` to satisfy DCO by default.
...
- If feedback becomes functionality-heavy or systemic, recommend `copilot-feedback-gate`.
- If fixes materially change milestone scope, recommend `milestone-conformance-review` and then `pre-pr-adversary-review`.
- If review threads are resolved and checks pass, recommend waiting for the user to merge; after merge, recommend `post-merge-cleanup`, then `initiative-completion` for initiative bookkeeping.
